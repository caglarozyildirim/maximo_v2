#!/usr/bin/env python3
"""
Add workflow diagrams to the analysis document
"""
...
# Workflow diagrams
WORKFLOWS = {
    'job-request': '/Users/caglarozyildirim/WebstormProjects/Deneme/diagrams/job-request-workflow.png',
    'asset': '/Users/caglarozyildirim/WebstormProjects/Deneme/diagrams/asset-registration-workflow.png',
    'maintenance': '/Users/caglarozyildirim/WebstormProjects/Deneme/diagrams/maintenance-duty-workflow.png',
    'incident': '/Users/caglarozyildirim/WebstormProjects/Deneme/diagrams/incident-workflow.png',
}

def create_document():
    """Create the complete document with workflows"""
    doc = Document()
    ...
    print("📄 Doküman oluşturuluyor (workflow'larla)...")
    ...
    # 3.1 Job Requests
    print("✍️  3.1 İş Talepleri (workflow ekleniyor)...")
    doc.add_heading('3.1 İş Talepleri Yönetimi', level=2)
    doc.add_paragraph('İş talepleri modülü, kullanıcıların bakım, onarım veya yeni kurulum taleplerini sisteme girmesini, onay sürecinden geçirmesini ve çözüm aşamasına kadar takip edilmesini sağlar. SLA takibi ile zamanında tamamlanma garanti edilir.')
    
    doc.add_heading('Müşteri Gereksinimi:', level=3)
    doc.add_paragraph(data['job_requests']['requirement'])
    
    doc.add_heading('İş Akışı Diyagramı:', level=3)
    doc.add_paragraph('İş talebi oluşturulmasından çözüme kadar olan süreç aşağıdaki iş akışı diyagramında detaylı olarak gösterilmiştir:')
    add_screenshot(doc, WORKFLOWS['job-request'])
    doc.add_paragraph()
    ...
    # 3.2 Assets
    print("✍️  3.2 Varlık Yönetimi (workflow ekleniyor)...")
    doc.add_heading('3.2 Varlık Yönetimi', level=2)
    doc.add_paragraph('Varlık yönetimi modülü, tüm sabit varlıkların sisteme kaydedilmesini, SAP ile entegrasyonunu, lokasyon takibini, zimmet işlemlerini ve yaşam döngüsü yönetimini sağlar.')
    
    doc.add_heading('Müşteri Gereksinimi:', level=3)
    doc.add_paragraph(data['assets']['requirement'])
    
    doc.add_heading('İş Akışı Diyagramı:', level=3)
    doc.add_paragraph('Varlık kayıt sürecinden SAP entegrasyonuna kadar olan süreç aşağıdaki iş akışı diyagramında detaylı olarak gösterilmiştir:')
    add_screenshot(doc, WORKFLOWS['asset'])
    doc.add_paragraph()
    ...
    # 3.3 Maintenance
    print("✍️  3.3 Bakım Yönetimi (workflow ekleniyor)...")
    doc.add_heading('3.3 Bakım Yönetimi', level=2)
    doc.add_paragraph('Bakım yönetimi modülü, periyodik ve planlı bakım işlemlerinin planlanmasını, görev listelerinin oluşturulmasını, teknisyen atamalarını ve bakım tamamlama sürecinin takibini sağlar.')
    
    doc.add_heading('Müşteri Gereksinimi:', level=3)
    doc.add_paragraph(data['maintenance']['requirement'])
    
    doc.add_heading('İş Akışı Diyagramı:', level=3)
    doc.add_paragraph('Bakım görevinin planlanmasından tamamlanmasına kadar olan süreç aşağıdaki iş akışı diyagramında detaylı olarak gösterilmiştir:')
    add_screenshot(doc, WORKFLOWS['maintenance'])
    doc.add_paragraph()
    ...
    # 3.4 Incidents
    print("✍️  3.4 Olay Yönetimi (workflow ekleniyor)...")
    doc.add_heading('3.4 Olay Yönetimi', level=2)
    doc.add_paragraph('Olay yönetimi modülü, acil arızaların ve beklenmedik olayların hızlı bir şekilde bildirilmesini, önceliklendirilmesini ve müdahale ekibinin atanmasını sağlar.')
    
    doc.add_heading('Müşteri Gereksinimi:', level=3)
    doc.add_paragraph(data['incidents']['requirement'])
    
    doc.add_heading('İş Akışı Diyagramı:', level=3)
    doc.add_paragraph('Olay bildiriminden çözüme kadar olan süreç aşağıdaki iş akışı diyagramında detaylı olarak gösterilmiştir:')
    add_screenshot(doc, WORKFLOWS['incident'])
    doc.add_paragraph()
